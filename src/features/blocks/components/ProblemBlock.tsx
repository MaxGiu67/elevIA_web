...
export function ProblemBlock({ statement, painPoints, className = '' }: ProblemBlockProps) {
  return (
    <div className={`bg-red-50 rounded-2xl p-8 ${className}`}>
      ...
      <ul className="space-y-3">
        {painPoints.map((point, index) => (
          <li key={index} className="flex items-start gap-3">
            <span className="w-6 h-6 rounded-full bg-red-100 text-red-600 flex items-center justify-center text-sm flex-shrink-0 mt-0.5">
              ✕
            </span>
